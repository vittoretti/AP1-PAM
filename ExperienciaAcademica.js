...
export default function ExperienciaAcademica() {
  ...
  //criação das variáveis para a "captura do texto"
   const [expacd, SetExpacd] = useState ('');
  ...
  return (
    ...
    <StatusBar backgroundColor="#4169E1" />
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <KeyboardAvoidingView
          ...
          <Text style={styles.title}> Tela de Cadastro </Text>
           <Text style={styles.label}> Escreva sobre sua experiência acadêmica.</Text>
            <Card style={styles.card}>
              <Text>Nome:</Text>
              <TextInput
                style={styles.input}
                placeholder="Universidade XXXXX, São Paulo, MM/AAAA"
                multiline={true}          
                numberOfLines={10}         
      ...
      <Button
      mode={'contained'}
      onPress={() => navigation.navigate('Exp.Profissional')} 
      icon='arrow-right'
      style={styles.button}
      >
      Próxima Tela
